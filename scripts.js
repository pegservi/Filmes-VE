...
document.addEventListener('DOMContentLoaded', function() {
    ...
    function discoverMovies(genreId, year, page) {
        ...
        loading = true;
        let apiUrl = `https://api.themoviedb.org/3/discover/movie?api_key=539facd7253c87259b2761897287f052&page=${page}&language=pt-BR&sort_by=popularity.desc&include_adult=false&include_video=true`;
        if (genreId) {
    ...
    function searchMovies(query) {
        ...
        loadingSpinner.style.display = 'block';
        loading = true;
        const apiUrl = `https://api.themoviedb.org/3/search/movie?api_key=539facd7253c87259b2761897287f052&query=${query}&language=pt-BR&page=1&include_adult=false`;
...
<div class="option-btn option-btn-active" data-link="https://embed.warezcdn.com/filme/${movie.imdb_id}">OPÇÃO 1</div>
...
<div class="blogger-entry-data" style="display: none;" entry-type="${movie.genres.map(genre => genre.name).join(', ')}" entry-bg="https://image.tmdb.org/t/p/w500${movie.poster_path}" entry-title="${movie.title}"></div>
    ...
    function getMovieDetails(title) {
        return fetch(`https://api.themoviedb.org/3/search/movie?api_key=539facd7253c87259b2761897287f052&query=${encodeURIComponent(title)}`)
            .then(response => response.json())
            .then(data => {
                const movie = data.results[0];
                return fetch(`https://api.themoviedb.org/3/movie/${movie.id}?api_key=539facd7253c87259b2761897287f052&append_to_response=videos`)
                    .then(response => response.json())
                    .then(details => {
